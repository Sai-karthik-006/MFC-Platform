...
import { AdminNotificationService, type AdminNotification } from "../../services/notification.service";
...
export function AdminNotificationBell() {
  ...
  const fetchNotifications = React.useCallback(async () => {
    ...
    try {
      const response = await AdminNotificationService.prototype.getAll();
  ...
  const fetchUnreadCount = React.useCallback(async () => {
    if (!accessToken) return;
    try {
      const response = await AdminNotificationService.prototype.getUnreadCount();
  ...
  const handleMarkAllAsRead = async () => {
    setIsLoading(true);
    try {
      await AdminNotificationService.prototype.markAllAsRead();
  ...
  const handleMarkAsRead = async (id: string) => {
    try {
      await AdminNotificationService.prototype.markAsRead(id);
